...
bool checkInvalidPattern(long id){
    ...
    int numLength{ static_cast<int>(number.length()) };

    if(numLength%2 != 0) return false;
    if(number.substr(0,(numLength/2)) == number.substr((numLength/2),(numLength/2))) return true;
    return false;
}
...
int main(){
    ...
    long result{ sumInvalidIDs(idRanges) };
